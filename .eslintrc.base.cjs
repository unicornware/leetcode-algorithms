...
const spec = require('./.eslintrc.spec.cjs')
...
const config = {
  ...
  rules: {
    ...
    'jsdoc/no-undefined-types': [
      ...
      {
        definedTypes: [
          ...spec.rules['jsdoc/no-undefined-types'][1].definedTypes,
          'NodeJS',
          'never',
          'unknown'
        ]
    ...
    'unicorn/prefer-object-from-entries': [2, { functions: ['fromPairs'] }],
  ...
  overrides: [
    ...
    {
      files: ['**/*.interface.ts', '**/*.type.ts', '**/*.types.ts'],
      rules: {
  ...
  settings: {
    'import/parsers': {
      '@typescript-eslint/parser': ['.cjs', '.d.ts', '.ts']
    },
    'import/resolver': {
      node: {
        extensions: ['.cjs', '.ts'],
